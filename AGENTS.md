...
A small local notebook for Erenshor (native Lunaris plugin, .NET Framework 4.8, C# 5 effective language level via `csc`). Freeform, player-owned, tabbed text pages with local autosave, plus an optional Chronicle view that other mods can append verified events to.
...
- **No networking.** Notes never leave the machine, and journal contents are never written to the Lunaris log.
- The current Lunaris build uses only narrow verified game/UI access for character identity, native `PlayerTyping` ownership, and fully-in-world readiness. Player-facing UI is retained Unity uGUI; it does not use Harmony click/camera containment. These reads must not be used to infer gameplay or broaden Journal into a game-state controller.
...
- Do not add Harmony/game hooks merely to make the retained uGUI work. Any future game hook needs an explicit design reason and current assembly evidence.
...
- `src/JournalStore.cs` — local persistence under `plugins/config/ErenshorJournal/journal.dat`, atomic replace, `.bak` and `.corrupt-*` recovery. Be careful here: never destroy an unreadable file, preserve it as a timestamped corrupt copy.
- `src/JournalWindow.cs`, `src/JournalLauncher.cs`, `src/RetainedUiKit.cs` — retained uGUI (draggable/resizable window, draggable launcher button, saved position/size, unload-safe gesture ownership).
...
- `src/ErenshorJournalPlugin.cs` — native Lunaris plugin entry point, character context, typing ownership, and retained-UI lifecycle.
- `src/JournalControlApi.cs`, `src/SuiteUiPolicy.cs` — optional Hub-facing primitive control surface and standalone UI readiness/fallback policy.
